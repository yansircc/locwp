...
sleep 3
assert_http_status "$(site_url a)" "200" "single-char site accessible"

# Numeric-only name
run_cmd "$BINARY" add 123 --pass a23456
assert_exit_ok $? "numeric name '123' allowed"
sleep 3
assert_http_status "$(site_url 123)" "200" "numeric site accessible"
...
sleep 3
assert_http_status "$(site_url a)" "200" "site works after idempotent ops"
...
assert_http_status "$(site_url 123)" "200" "123 accessible before deletion"
...
sleep 3
assert_http_status "$(site_url 123)" "200" "123 accessible after re-add"
...
if [[ ! -d "$LOCWP_HOME/sites/123" ]]; then
  pass "stopped site directory deleted (including SQLite DB)"
else
...
sleep 3
assert_http_status "$(site_url cycle)" "200" "site accessible after cycles"
...
rm -f "$LOCWP_HOME/caddy/sites/broken.caddy"*
rm -f "$LOCWP_HOME/php/broken.conf"
rm -f "$BREW_PREFIX/etc/php/8.3/php-fpm.d/locwp-broken.conf"

# 7b: stop/start after manually deleting caddy conf
run_cmd "$BINARY" add ghost --pass a23456
assert_exit_ok $? "create ghost site"

rm -f "$LOCWP_HOME/caddy/sites/ghost.caddy"
run_cmd "$BINARY" stop ghost
pass "stop does not crash after caddy conf deleted (exit $?)"

run_cmd "$BINARY" start ghost
pass "start does not crash after caddy conf deleted (exit $?)"

"$BINARY" delete ghost 2>/dev/null || true

# 7c: manually delete caddy conf and reload
run_cmd "$BINARY" add orphan --pass a23456
assert_exit_ok $? "create orphan site"

rm -f "$LOCWP_HOME/caddy/sites/orphan.caddy"
brew services restart caddy 2>/dev/null || true
pass "caddy reload does not crash after conf deleted"

"$BINARY" delete orphan 2>/dev/null || true

# ════════════════════════════════════════════════════════
# Edge Case 8: SQLite database isolation
# ════════════════════════════════════════════════════════
echo ""
echo -e "${CYAN}━━━ Edge 8: SQLite database isolation ━━━${NC}"

run_cmd "$BINARY" add isolated --pass a23456
assert_exit_ok $? "add isolated site"

# Verify SQLite DB is inside the site directory
assert_file_exists "$LOCWP_HOME/sites/isolated/wordpress/wp-content/database/.ht.sqlite" "SQLite DB exists in site dir"

# Delete site and verify DB is gone
"$BINARY" delete isolated 2>/dev/null || true
if [[ ! -f "$LOCWP_HOME/sites/isolated/wordpress/wp-content/database/.ht.sqlite" ]]; then
  pass "SQLite DB cleaned up with site deletion"
else
  fail "SQLite DB should be removed with site"
fi
...
# ════════════════════════════════════════════════════════
# Edge Case 12: port uniqueness
# ════════════════════════════════════════════════════════
echo ""
echo -e "${CYAN}━━━ Edge 12: port uniqueness ━━━${NC}"
...
port1=$(site_port test1)
port2=$(site_port test2)
if [[ "$port1" != "$port2" ]]; then
  pass "test1 and test2 have different ports ($port1 vs $port2)"
else
  fail "test1 and test2 should have different ports (both $port1)"
fi

sleep 3
assert_http_status "$(site_url test1)" "200" "test1 accessible"
assert_http_status "$(site_url test2)" "200" "test2 accessible"
...
if [[ $rc -eq 0 ]]; then
  assert_contains "$wp_out" "localhost" "wp option get returns localhost URL"
else
