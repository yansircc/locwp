	...
	workflows := map[string]workflowDef{
		"provision": {
			...
			steps: []pawlStep{
				...
				{Name: "create-db", Run: "mariadb --no-defaults -u ${db_user} -e 'CREATE DATABASE IF NOT EXISTS `${db_name}`'", OnFail: "retry"},
				{Name: "download-wp", Run: "PHPRC= ${php_bin} -d memory_limit=512M $(which wp) core download --path=${wp_root} --version=${wp_ver}", OnFail: "retry"},
				{Name: "gen-wp-config", Run: "PHPRC= ${php_bin} -d memory_limit=512M $(which wp) config create --path=${wp_root} --dbname=${db_name} --dbuser=${db_user} --dbhost=${db_host} --skip-check"},
				{Name: "provision-services", Run: "brew services restart php@${php_ver} 2>/dev/null; sudo nginx -s reload", OnFail: "retry"},
				{Name: "install-wp", Run: "PHPRC= ${php_bin} -d memory_limit=512M $(which wp) core install --path=${wp_root} --url=https://${domain} --title=${site} --admin_user=${admin_user} --admin_password=${admin_pass} --admin_email=${admin_email}", OnFail: "retry"},
				{Name: "set-permalinks", Run: "PHPRC= ${php_bin} -d memory_limit=512M $(which wp) rewrite structure '/%postname%/' --path=${wp_root} && PHPRC= ${php_bin} -d memory_limit=512M $(which wp) rewrite flush --path=${wp_root}"},
			},
		},
		"start": {
		...
		"destroy": {
			...
			steps: []pawlStep{
				{Name: "drop-db", Run: "mariadb --no-defaults -u ${db_user} -e 'DROP DATABASE IF EXISTS `${db_name}`'"},
